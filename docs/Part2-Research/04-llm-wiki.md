...
> 이 문서를 Claude에게 보여주면 수집한 3편의 논문을 Karpathy 방식의 지식 베이스로 구축해줍니다.
...
| 03에서 수집한 산출물 | 논문 PDF 3편 + paper-list.md | `ls primary/papers/` |

> [에이전트 팀 문헌 수집](./03-collection.md)을 먼저 완료하세요. `primary/papers/`에 PDF 3편과 `primary/paper-list.md`가 있어야 합니다.
...
> 상세 아키텍처(Hooks 자동화, RAG 비교, 코딩 세션 활용)는 Part4-Appendix의 LLM-Wiki 상세 가이드를 참고하세요 (로컬 전용).


## 1. 프로젝트 구조 만들기

01에서 열어둔 starter-kit이 이미 3계층 구조를 갖추고 있습니다:
...
└── work_paper/                  # 06에서 사용할 LaTeX 집필 공간
    ├── main.tex
    └── references.bib
```
...
## 프로젝트 목적
LLM-Based Code Generation 논문 3편의 지식 베이스를 구축하고,
...
- `work_paper/` — LaTeX 집필 공간. 06 단계에서 사용.
...
```
primary/papers/ 의 논문 3편을 읽고 지식 베이스를 구축해줘.

[요약 페이지]
각 논문마다 secondary/summaries/ 에 요약 페이지를 만들어줘.
- 파일명: {모델명}_summary.md (예: codex_summary.md)
- 내용: 목적, 방법론, 주요 결과(수치 포함), 한계, 핵심 기여
- Frontmatter: title, type(paper), status(📋 대기), authors, year,
  approach, dataset, performance, sources, related, last_compiled

[개념 페이지]
논문들의 공통 접근법이나 개념은 secondary/concepts/ 에 통합 페이지로 만들어줘.
- 파일명: {개념-키워드}.md (예: fine-tuning.md, benchmark-evaluation.md)
- Frontmatter: title, type(concept), sources, related, last_compiled

[연결과 목록]
- 페이지 간 [[wiki-links]]로 연결해줘
- secondary/index.md 에 전체 목차를 만들어줘
- secondary/log.md 에 작업 기록을 남겨줘 (append-only)
...
1. 논문 제목 (Title) — 주제와 비교 관점이 드러나는 영문 제목
2. 섹션 구조 — 각 섹션마다 다룰 핵심 내용 2~3줄 요약
   - Introduction: 코드 생성 분야 개관
   - Background: 주요 벤치마크와 평가 방법
   - Approaches: 접근법별 비교 분석
   - Discussion: 한계와 향후 연구 방향
   - Conclusion
3. Figure 1개 — 아래 항목을 모두 정해줘:
   - 제목 (예: "Figure 1: Comparison of Code Generation Approaches")
   - 내용 설명 (무엇을 보여주는 그림인지)
   - 들어갈 섹션
   - 제작 방식: 아래 중 하나를 선택하고 이유도 적어줘
     | 방식 | 적합한 경우 |
     | TikZ | 아키텍처 다이어그램, 플로우차트 (LaTeX 내장) |
     | matplotlib | 수치 그래프, 바 차트, 성능 추이 (Python) |
     | Mermaid → PNG | 간단한 흐름도 (mmdc CLI 변환) |
     | 직접 그리기 | draw.io 등으로 수작업 |
4. Table 1개 — 아래 항목을 모두 정해줘:
   - 제목 (예: "Table 1: Benchmark Performance Comparison")
   - 행/열 구성
   - 들어갈 섹션
   - 데이터 출처 (paper_comparison.md 기반인지, 원본 논문에서 추출인지)

YAML Frontmatter에 Figure/Table 메타데이터도 포함해줘:
  fig_title, fig_section, fig_method, tbl_title, tbl_section

secondary/outline.md 에 저장해줘.
index.md 와 log.md 도 업데이트해줘.
```

아웃라인에 Figure/Table 계획이 포함되어야 06에서 본문 작성 시 그림과 표 위치를 미리 잡을 수 있습니다. Frontmatter의 메타데이터는 05 Obsidian 대시보드에서 자동으로 읽어 표시됩니다.
...
| 파일 | 역할 | 다음 단계 |
|------|------|----------|
| `secondary/paper_comparison.md` | 비교표 (SSOT) | 06에서 LaTeX 테이블로 변환 |
| `secondary/outline.md` | 논문 아웃라인 | 06에서 섹션별 집필 가이드 |
| `secondary/summaries/*.md` | 개별 논문 요약 | 06에서 인용 근거로 참조 |
| `secondary/concepts/*.md` | 접근법 통합 페이지 | 06에서 비교 분석 섹션 근거 |
...
→ **다음 단계**: 위키를 Obsidian으로 시각화하고 연구 대시보드를 만듭니다 — [Obsidian으로 연구 관리](./05-obsidian.md)
