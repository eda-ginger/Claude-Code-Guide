...
`my-mini-review/` 폴더 전체를 Obsidian 볼트로 엽니다.

```
Obsidian → 볼트 열기 → ~/my-mini-review 선택
```
...
dataset: "HumanEval"
performance: 67.8
sources: [primary/papers/03_CodeLlama_2023.pdf]
related: [[fine-tuning]], [[benchmark-evaluation]]
...
06에서 LaTeX 논문을 작성한 뒤, 결과를 Obsidian으로 확인하고 보완하는 사이클:

```
06에서 논문 작성 → Obsidian에서 확인
     ↓
수치 불일치 발견? → Lint 재실행 → SSOT 수정
     ↓
새로운 인사이트? → secondary/에 새 페이지로 저장 (filing back)
     ↓
그래프 확장 → 연결 부족한 부분 발견 → 추가 분석
```
...
> 💡 **Tip:** 일상용 대시보드(할 일, 피드, PARA 구조)도 함께 사용하려면 [Gmail 피드 + Obsidian 대시보드](../Part3-Life/01-gmail-feed.md)을 참고하세요. 연구 프로젝트와 일상 관리를 하나의 볼트에서 통합할 수 있습니다.
