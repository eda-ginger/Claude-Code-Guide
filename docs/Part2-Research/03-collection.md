...
> 이 문서를 Claude에게 보여주면 에이전트 팀으로 논문을 수집하고 핵심 3편을 선별해줍니다.
>
...
2명의 에이전트가 각각 다른 검색 엔진을 담당합니다. API 키가 필요 없어 바로 시작할 수 있습니다.

```
[Leader/Conductor] ─┬─ [Teammate A: arXiv] → CS preprint (LLM 논문 대부분 여기)
                    └─ [Teammate B: DBLP]  → CS 학회/저널 논문
```
...
> **규모가 크다면**: NotebookLM(nlm CLI), Semantic Scholar, PubMed 에이전트를 추가할 수 있습니다.
...
```
논문 수집 팀을 만들어줘. 2명의 팀원이 각각 다른 검색 엔진을 담당해:
...
각 팀원은 결과를 secondary/ 폴더에 자기 파일로 저장해 (secondary/arxiv.md, secondary/dblp.md).
...
Teammate A ──"arXiv에서 'code infilling' 키워드로 관련 논문 3편 추가 발견"
    ──► Leader ──"이 키워드로도 검색해봐"──► Teammate B
```
...
```
Teammate A ──"arXiv 완료: 15편"──► Leader ──"전원 완료 확인. 병합 시작할게"
Teammate B ──"DBLP 완료: 8편"───►
```
...
리더(또는 사용자)가 팀원의 결과를 취합하고, 미니 리뷰에 사용할 핵심 3편을 선별합니다.
...
3. 동일 논문이 여러 에이전트에서 발견 → Source에 모두 기재 (예: "A,B")
```
...
중복 제거와 스코프 필터링 후, 아래 **4가지 기준을 모두 충족**하는 **핵심 3편**을 선별합니다:

| 기준 | 설명 |
|------|------|
| 높은 인용수 | 인용수가 높은 논문 우선 (영향력이 검증된 핵심 논문) |
| 서로 다른 접근법 | 3편이 각각 다른 접근법을 대표 (아래 예시 참고) |
| 모델 논문 | 새로운 모델을 제안하는 논문 (서베이·분석 논문이 아닌 모델 자체 논문) |
| 비교 가능한 벤치마크 | 동일 벤치마크(HumanEval, MBPP 등) 결과가 있어 성능 비교 가능 |

3편만 선별하므로 접근법 다양성이 특히 중요합니다. 예를 들어 LLM Code Generation 분야라면 이런 축에서 겹치지 않게 고릅니다:
...
선별된 3편은 arXiv/OA 논문이므로 `curl`로 바로 받을 수 있습니다.
...
→ **다음 단계**: 수집한 3편으로 지식 베이스를 구축합니다 — [LLM-Wiki](./04-llm-wiki.md)
